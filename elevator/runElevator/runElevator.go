		...
		*/
		case <-doorTimeoutSignal.C:
			...
			case elevator.EB_DoorOpen:
				...
				case elevator.EB_DoorOpen:
					switch {
					case localElevator.Dirn == elevio.MD_Up:
						if !requests.RequestsAbove(localElevator) {
							localElevator.Requests[localElevator.Floor][elevio.BT_HallDown] = false

						}
					case localElevator.Dirn == elevio.MD_Down:
						if !requests.RequestsBelow(localElevator) {
							localElevator.Requests[localElevator.Floor][elevio.BT_HallUp] = false
						}
					}
					...
					if localElevator.Requests[localElevator.Floor][0] {
						chClearedHallRequests <- requests.RequestClearHallRequestsAtCurrentFloor(localElevator, specialCase)
			...
			case elevator.EB_Moving:
				if requests.RequestsShouldStop(localElevator) {
					...
					if localElevator.Requests[localElevator.Floor][0] && localElevator.Requests[localElevator.Floor][1] {
						...
						chClearedHallRequests <- requests.RequestClearHallRequestsAtCurrentFloor(localElevator, specialCase) //flyttet fra 360
						localElevator = requests.RequestsClearAtCurrentFloor(localElevator)
						doorTimeoutSignal.Reset(3 * time.Second)
